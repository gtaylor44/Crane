...
{
    public class Select10<T> : AbstractSelect
    {
        public Select10(List<ISprocObjectMap> sprocObjectMapList) : base(sprocObjectMapList){}

        public Select10<T> AddSqlParameter(SqlParameter item)
        {
            ParamList.Add(item);
            return this;
        }

        public Select10<T> AddSqlParameter(string parameterName, SqlDbType dbType, object value)
        {
            ParamList.Add(new SqlParameter(parameterName, dbType) { Value = value });
            return this;
        }

        public List<T> ExecuteReader<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>(SqlConnection conn, string procName, Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T> callBack,
