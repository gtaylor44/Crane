...
namespace SprocMapperLibrary
{
    public class Select9<T> : AbstractSelect
    {
        ...
        public Select9<T> AddSqlParameter(SqlParameter item)
        {
            ParamList.Add(item);
            return this;
        }

        public Select9<T> AddSqlParameter(string parameterName, SqlDbType dbType, object value)
        {
            ParamList.Add(new SqlParameter(parameterName, dbType) { Value = value });
            return this;
        }

        public List<T> ExecuteReader<T1, T2, T3, T4, T5, T6, T7, T8, T9>(SqlConnection conn, string procName, Func<T1, T2, T3, T4, T5, T6, T7, T8, T9, T> callBack,
